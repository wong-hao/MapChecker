...
namespace SMGI.Plugin.CartographicGeneralization
{
    ...
    /// 符号混用检查
    /// </summary>
    public class SymbolMatchCheckCmd : SMGICommand
    {
        ...
        {
            outputFileName = OutputSetup.GetDir() + string.Format("\\{0}.shp", "符号混用检查");
            ...
            try
            {
                ...
                foreach (var tt in tts)
                {
                    ...
                    try
                    {
                        ...
                        errList.Add(CheckSymbolMatch(ptFC, ptQF, plFC, plQF));
                    }
                    catch (Exception ex)
                    {
                ...
                if (errList.Count > 0)
                {
                    ...
                    foreach (var item in errList)
                    {
                        foreach (var itemInter in item)
                        {
                            ...
                            fieldName2FieldValue.Add("检查项", "符号混用检查");
        ...
        public static List<Tuple<IGeometry, string, int, string, int>> CheckSymbolMatch(
